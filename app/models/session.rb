class Session < ActiveRecord::Base
  ...
  def my_results(speed_accuracy, style, color)
			
		if speed_accuracy == 'speed'
			...
			if style == 'both' && color == 'both'
				...
				#all total
				return [(self.trials.average('task_time').to_f / 1000).round(3)]
			...
			if style == 'both' && color == 'both'
				...
				#all total
				return [(self.trials.where(task_success: true).count.to_f / 0.24).round(1)]
